...
public class ReceiveExpressPageActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE,
				R.layout.sign_scan_title);

		TextView text = (TextView) findViewById(R.id.left_text);
		text.setText(R.string.receive_no_order);
		...
		mScanManager.registerResultListener(new ScanResultListener() {
			@Override
			public void onScan(ScanManager arg0, byte[] sanResult) {
				if (sanResult != null) {
					String barcode = new String(sanResult);
					onScanned(barcode);
				}
			}
		});
