...
public class OrderManager {
	...
	//订单状态
	public static final Integer STATE_INIT = 0;//未取（未读，未上传）
	public static final Integer STATE_FETCHED = 1;//已取(正常,已读,未上传）
	public static final Integer STATE_VIEWED = 2;//已查看
	//本地的订单状态
	public static final Integer STATE_INIT_READED = 3;//未取（已读，未上传）
	public static final Integer STATE_FETCHED_UPLOADED = 4;//已取(正常,已读,已上传）
	public static final Integer STATE_FETCHED_EXCEPTION = 5;//已取（异常，已读，未上传）
	public static final Integer STATE_FETCHED_UPLOADED_EXCEPTION = 6;//已取（异常，已读，已上传）
	public static final Integer STATE_CANCELED = 7;//订单已作废（未读，未上传）
	public static final Integer STATE_CANCELED_READED = 8;//订单已作废（已读，未上传）
	public static final Integer STATE_CANCELED_UPLOADED = 9;//订单已作废（已读，已上传）
	public static final Integer STATE_ENERGED = 10;//催办订单（未读，未上传）
	public static final Integer STATE_ENERGED_READED = 11;//催办订单（已读，未上传）
	public static final Integer STATE_ENERGED_UPLOADED = 12;//催办订单（已读，已上传）
	private AppContext mAppContext;
	private DownloadOrderRequestMsgVO mOrderRequestMsgVO;
	private ModifyOrderStatusRequestMsgVO mOrderStatusRequestMsgVO;
//	private LocalOrderVO mOrderVO;
//	private List<LocalOrderVO> mListOrderVO;
...
//		mOrderVO = new LocalOrderVO();
//		mListOrderVO = new ArrayList<LocalOrderVO>();
		try {
	...
	public boolean downloadOrder(Context context) {
		LogUtils.d(TAG, "start to download orders");
	...
	public boolean modifyOrderStatus(Context context, Listener mListener){
		LogUtils.d(TAG, "start to modify order status");
	...
	private void addOrder(OrderVO orderVO){
		LogUtils.d(TAG, orderVO.toString());
		try {
...
//			list = mOrderDao.queryForAll();
	...
	private OrderVO queryOrderVO(String orderNo){
		OrderVO local = null;
		try {
//			List<OrderVO> list = mOrderDao.queryForEq("orderNo", orderNo);
